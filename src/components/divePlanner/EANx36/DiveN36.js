...
import { GetPGN36 } from "./GetPGN36";
import { GetRNTN36 } from "./GetRNTN36";

export const DiveN36 = ({ step, dives, updateDive2, updateDive3, resetDives, resetAllDives, setAndUpdate }) => {
    ...
    const getRNTandRunPG = () => {
        GetRNTN36(step, diveInput.depth, currentDive.pgAfterSi);
        GetPGN36(diveInput, step, currentDive, setAndUpdate)
    }

    const diveOnEnter = (event) => {
        if (event.code === "Enter" || event.code === "NumpadEnter") {
            if (step === 1) { GetPGN36(diveInput, step, currentDive, setAndUpdate) }
    ...
    return (
        <>
            ...
            <section className="inputs-flagN36">
                <div className="divePlanInputDivN36">
                    {currentDive.pg !== '' ? '' :
                        <>
                            {step !== 1 ?
                                <><fieldset>
                            ...
                            <div className="diveButtons">
                                {step === 1
                                    ? <button
                                        onClick={() => GetPGN36(diveInput, step, currentDive, setAndUpdate)}>Dive</button>
            ...
            {/* RESULTS */}
            <section className="diveResultsN36">
                {step === 2 || step === 3 ?
                    <ul className="diveResultsListN36">
                        {diveInput.si > 0 ?
                            <><li>
                                <div className="results-labelN36">Surface Interval:</div> {diveInput?.si} min</li>
                                <li>
                                    <div className="results-labelN36">Starting Pressure Group:</div><div className="pg-box">{currentDive?.pgAfterSi}</div>
                                </li></>
                            : ''
                        }
                    </ul>
                ...
                {currentDive.depth !== 0 ?
                    <ul className="diveResultsListN36">
                        <li>
                            <div className="results-labelN36">Pressure group:</div> {currentDive.pg !== '' ? <div className="pg-boxN36">{currentDive.pg}</div> : ''}
                        </li>
                        <li>
                            <div className="results-labelN36">Depth:</div> {currentDive.depth > 0 ? currentDive.depth + ' feet' : ''}
                        </li>
                        <li>
                            <div className="results-labelN36">pp02:</div> {currentDive.ppO2.toFixed(2)}
                        </li>
                        {step === 1 ?
                            <li>
                                <div className="results-labelN36">Total bottom time:</div>
                        ...
                        {step === 2 || step === 3 ?
                            <><div className={currentDive.noDecoLimit === false && currentDive.ssRequired === false ? "timeGreen"
                                ...
                                <li>
                                    <div className="results-labelN36">&nbsp;&nbsp;Actual bottom time:</div>
                                    {currentDive.abt > 0 ? currentDive.abt + ' min' : ''}
                                </li>
                                <li>
                                    <div className="results-labelN36">&nbsp;&nbsp;+ Residual nitrogen time:</div>
                                    {currentDive.rnt} min
                                </li>
                                <li>
                                    <div className="results-labelN36">&nbsp;&nbsp;= Total bottom time: </div>
                        ...
                        <li>
                            <div className="results-labelN36">Safety stop required:</div>
                        ...
                        <li>
                            <div className="results-labelN36">Pre-flight surface interval: </div>
                            {step !== 1 ? `18 hours` : currentDive.noDecoLimit ? `18 hours` : `12 hours`}
                        </li>

                        {/* Min over no deco limit */}
                        {currentDive.minOverDeco < 0 ?
                            <><li>
                                <div className="results-labelN36">Minutes to no deco limit: </div>
                        ...
                        }
                        {
                            currentDive.ppO2 > 1.39 ?
                                <li>
                                    <div className="noDecoLimitN36">
                                        <b>❗ ppO2 exceeded.</b> This dive is highly discouraged. The maximum depth for EANx36 is exceeded by {currentDive.depth - 90} feet which greatly increases the risk of oxygen toxicity, convulsions and drowning. Do not descend past the maximum depth of 90 feet.
                                    </div>
                                </li> : ''
                        }
                        ...
                        {currentDive.minOverDeco > 0 ?
                            <>
                                <li><div className="results-labelN36">Min over no deco limit:</div> {currentDive.minOverDeco}</li>
                                <li>
                                    <div className="noDecoLimitN36">
